...
export default function RecipientsExisting() {
  ...
  useEffect(() => { if (secondaryCount === 0) navigate("/existing-customer-payment", { replace: true }); }, [secondaryCount, navigate]);
  ...
  const submit = (e) => { e.preventDefault(); const errs = checkDuplicates(users); setErrors(errs); if (Object.keys(errs).length) return; localStorage.setItem("rfqWatchSetup", JSON.stringify({ ...setup, secondaryUsers: users })); navigate("/existing-customer-payment"); };
  return <FlowLayout step={2} title="Add Recipients" subtitle={`Add the ${secondaryCount} additional ${secondaryCount === 1 ? "recipient" : "recipients"} who should receive RFQ alerts.`} infoProps={{ title: "Keep Your", accent: "Whole Team", ending: "Informed.", copy: "Add secondary recipients to your RFQ Watch account to keep your whole team up to speed on the latest RFQs relevant to your business." }}>
    <form className="rfq-form" onSubmit={submit}>
      ...
      <div className="form-divider" /><div className="form-actions"><button type="button" className="back-button" onClick={() => { localStorage.setItem("rfqWatchSetup", JSON.stringify({ ...setup, secondaryUsers: users })); navigate("/create-account-existing"); }}><ArrowLeft size={18} /> Back</button><button className="continue-button" type="submit">Continue to Payment <ArrowRight size={18} /></button></div>
    </form>
  </FlowLayout>;
}