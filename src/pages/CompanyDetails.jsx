...
export default function CompanyDetails() {
  ...
  return <FlowLayout step={2} title="Add Secondary Users" subtitle={`Add the ${secondaryCount} additional ${secondaryCount === 1 ? "user" : "users"} who should receive RFQ alerts.`} infoProps={{ title: "Keep Your", accent: "Whole Team", ending: "Informed." }}>
    <form className="rfq-form" onSubmit={submit}>
      <div className="secondary-list">{users.map((user, index) => <fieldset className="user-box" key={index}><legend>User {index + 2}</legend><div className="form-grid">
